...
The data directory is the default location where the DogecoinEV Core files are stored.
...
Windows  | `%APPDATA%\DogecoinEV\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `dogecoinev.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `dogecoinev.conf`       | User-defined configuration settings for `dogecoinevd` or `dogecoinev-qt`; can be specified by `-conf` option
`./`               | `dogecoinevd.pid`        | Stores the process ID (PID) of `dogecoinevd` or `dogecoinev-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `dogecoinevd` or `dogecoinev-qt`
...
`dogecoinev-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by DogecoinEV Core:
