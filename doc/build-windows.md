...
Below are some notes on how to build DogecoinEV Core for Windows.
...
DogecoinEV GitHub repository](https://github.com/dogecoinevppc/dogecoinev/)).
...
Note that for WSL v1 the DogecoinEV Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dogecoinev, AND not under, for example, /mnt/d/dogecoinev. 
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for DogecoinEV's dependencies):
...
way. This will install to `c:\workspace\dogecoinev`, for example:

    make install DESTDIR=/mnt/c/workspace/dogecoinev
