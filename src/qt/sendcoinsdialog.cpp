...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    // DogecoinEV: We don't use smart fees in the UI, so don't need to warn they're not available
...
void SendCoinsDialog::updateGlobalFeeVariables()
{
    if (ui->radioSmartFee->isChecked())
    {
        ...
        // DogecoinEV: We manually set height well past the last hard fork here
        ui->confirmationTargetLabel->setText(GetDogecoinEVPriorityLabel(nPriority).c_str());
    }
    else
    {
...
void SendCoinsDialog::updateFeeLabel()
{
    ...
    int nPriority = ui->sliderSmartFee->value();
    CFeeRate feeRate = GetDogecoinEVFeeRate(nPriority);
    ...
    else
    {
        ...
        // DogecoinEV: We don't use smart fees, so we don't have the data to estimate when it will get in
        ui->labelFeeEstimation->setText("");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid DogecoinEV address"));
        }
        else // Valid address
        {
            CKeyID keyid;
