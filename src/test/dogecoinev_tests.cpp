...
BOOST_FIXTURE_TEST_SUITE(dogecoinev_tests, TestingSetup)
...
uint64_t expectedMaxSubsidy(int height) {
    if (height < 100000) {
        return 1000000 * COIN;
    } else if (height < 145000) {//ELON TODO Magic number
        return 500000 * COIN;
...
BOOST_AUTO_TEST_CASE(subsidy_test)
{
    ...
    for (int nHeight = 0; nHeight < 600000; nHeight++) {
        ...
        CAmount nSubsidy = GetDogecoinEVBlockSubsidy(nHeight, params, prevHash);
    ...
    CAmount nConstantSubsidy = GetDogecoinEVBlockSubsidy(600000, mainParams.GetConsensus(600000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetDogecoinEVBlockSubsidy(700000, mainParams.GetConsensus(700000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e0fffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c271b09);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    pindexLast.nHeight = 145000;//ELON TODO Magic number
    pindexLast.nTime = 1395094679;
    pindexLast.nBits = 0x1b499dfd;
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}

BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    SelectParams(CBaseChainParams::MAIN);
    const Consensus::Params& params = Params().GetConsensus(1000);//ELON TODO Magic number
    ...
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateDogecoinEVNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
...
BOOST_AUTO_TEST_CASE(hardfork_parameters)
{
    ...
    const Consensus::Params& digishieldParams = Params().GetConsensus(1000);//ELON TODO Magic number
    ...
    const Consensus::Params& auxpowParams = Params().GetConsensus(42000);//ELON TODO Magic number
    BOOST_CHECK_EQUAL(auxpowParams.nHeightEffective, 42000);//ELON TODO Magic number
