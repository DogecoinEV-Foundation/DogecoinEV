...
const char * const BITCOIN_CONF_FILENAME = "dogecoinev.conf";
const char * const BITCOIN_PID_FILENAME = "dogecoinevd.pid";
...
#else
    const char* pszModule = "dogecoinev";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DogecoinEV";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DogecoinEV";
#else
    // Unix
    return pathRet / ".dogecoinev";
#endif
#endif
}
