...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "    \"feefilter\": \"decimal\",  (string)  The value of the last feefilter set by this peer, in ELON\n"
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:33874\", (string) The dogecoinev server IP and port we're connected to\n"
