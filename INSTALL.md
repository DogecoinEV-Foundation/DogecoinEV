# Installing DogecoinEV Core

### Pre-compiled binaries

The easiest way to install the latest version of the DogecoinEV Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/dogecoinevppc/dogecoinev/releases). Currently,
...
It is possible to build your own copy of DogecoinEV Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build DogecoinEV using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build DogecoinEV on your native
  ...
  - [macOS Build Notes](doc/Building-DogecoinEV-1.14-for-Mac.md)
...
Run with the `-testnet` option to run with "play dogecoinevs" on the test network, if you
...
DogecoinEV Core is a multithreaded application, and deadlocks or other multithreading bugs
