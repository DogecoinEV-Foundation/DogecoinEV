...
# ELON TODO Change these urls to dogecoinev.org and add a depends subdomain which will do what this does
# Dependencies
...
SIGNER=
VERSION=
url=https://github.com/dogecoinevppc/dogecoinev
proc=2
mem=2000
...
Standalone script to perform the gitian build of DogecoinEV Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/dogecoinevppc/dogecoinev
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/dogecoinev-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/dogecoinev-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean dogecoinev git directory because of old caching
    if [ -d inputs/dogecoinev/ ]; then
        echo "Cleaning DogecoinEV directory..."
        rm -rf inputs/dogecoinev/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit dogecoinev="$COMMIT" --url dogecoinev="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
